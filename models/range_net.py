...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, horizontal_stride=False):
        super(ResNet, self).__init__()
        self.inplanes = 64
        self.stride = [1, 2] if horizontal_stride else 2
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=self.stride, padding=3, bias=False)
        self.bn1 = BatchNorm(64)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=self.stride, padding=1)
...
class ResUnet(nn.Module):
    def __init__(self, in_channel, layers=18, classes=20, BatchNorm=nn.BatchNorm2d, pretrained=True):
        ...
        self.layer0 = inconv(in_channel, 64, stride=2)
        self.layer1, self.layer2, self.layer3, self.layer4 = resnet.layer1, resnet.layer2, resnet.layer3, resnet.layer4

        # Decoder
        # self.up4 = nn.Sequential(nn.ConvTranspose2d(512,256,kernel_size=2,stride=2),BatchNorm(256),nn.ReLU())
        self.up4 = nn.Sequential(nn.Conv2d(512*block.expansion, 256*block.expansion, kernel_size=3, stride=1, padding=1), BatchNorm(256*block.expansion), nn.ReLU())
        resnet.inplanes = 256*block.expansion+ 256*block.expansion
        self.delayer4 = resnet._make_layer(block, 256, layers[-1])

        self.up3 = nn.Sequential(nn.Conv2d(256*block.expansion, 128*block.expansion, kernel_size=3, stride=1, padding=1), BatchNorm(128*block.expansion), nn.ReLU())
        resnet.inplanes = 128*block.expansion + 128*block.expansion
        self.delayer3 = resnet._make_layer(block, 128, layers[-2])

        self.up2 = nn.Sequential(nn.Conv2d(128*block.expansion, 64*block.expansion, kernel_size=3, stride=1, padding=1), BatchNorm(64*block.expansion), nn.ReLU())
        resnet.inplanes = 64*block.expansion + 64*block.expansion
        self.delayer2 = resnet._make_layer(block, 64, layers[-3])

        self.cls = nn.Sequential(
            nn.Conv2d(64*block.expansion, 256, kernel_size=3, padding=1, bias=False),
            BatchNorm(256),
