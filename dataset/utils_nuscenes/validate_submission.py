...
def validate_submission(nusc: NuScenes,
    ...
    if verbose:
        print(f'Checking if folder structure of {results_folder} is correct...')

    # Check that {results_folder}/{eval_set} exists.
    results_meta_folder = os.path.join(results_folder, eval_set)
    assert os.path.exists(
        results_meta_folder
    ), f'Error: The folder containing the submission.json ({results_meta_folder}) does not exist.'
    ...
    assert os.path.exists(
        submisson_json_path
    ), f'Error: submission.json ({submisson_json_path}) does not exist.'
    ...
    assert os.path.exists(
        results_bin_folder
    ), f'Error: The folder containing the .bin files ({results_bin_folder}) does not exist.'
    ...
    if verbose:
        print(f'Checking contents of {submisson_json_path}...')
    ...
    with open(submisson_json_path) as f:
        ...
        assert valid_meta == set(
            submission_meta['meta'].keys()
        ), f'{submisson_json_path} must contain {valid_meta}.'

        for meta_key in valid_meta:
            meta_key_type = type(submission_meta['meta'][meta_key])
            assert (
                meta_key_type == bool
            ), f'Error: Value for {meta_key} should be bool, not {meta_key_type}.'
    ...
    if verbose:
        print(f'Checking if all .bin files for {eval_set} exist and are valid...')
    ...
    for sample_token in tqdm(sample_tokens, disable=not verbose):
        ...
        lidarseg_pred_filename = os.path.join(
            results_bin_folder, f'{sd_token}_lidarseg.bin'
        )

        assert os.path.exists(
            lidarseg_pred_filename
        ), f'Error: The prediction .bin file {lidarseg_pred_filename} does not exist.'
        ...
        if len(nusc.lidarseg) > 0:  # If ground truth exists, compare the no. of predictions with that of ground truth.
            ...
            assert os.path.exists(
                lidarseg_label_filename
            ), f'Error: The ground truth .bin file {lidarseg_label_filename} does not exist.'
        ...
        assert num_points == len(
            lidarseg_pred
        ), f'Error: There are {len(lidarseg_pred)} predictions for lidar sample data token {sd_token} but there are only {num_points} points in the point cloud.'


        assert all(
            (lidarseg_pred > 0) & (lidarseg_pred < num_classes)
        ), f"Error: Array for predictions in {lidarseg_pred_filename} must be between 1 and {num_classes - 1} (inclusive)."
    ...
    if verbose:
        print(f'Results folder {results_folder} successfully validated!')

    # Zip up results folder if desired.
    if zip_out:
        assert os.path.exists(
            zip_out
        ), f'Error: The folder {zip_out} to zip the results to does not exist.'
        ...
        if verbose:
            ...
            print(f'Results folder {results_folder} zipped to {results_zip_name}')
